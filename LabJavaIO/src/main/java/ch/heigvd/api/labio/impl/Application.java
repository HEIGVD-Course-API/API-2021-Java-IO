...
public class Application {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    try {
      OutputStreamWriter out = new OutputStreamWriter(new FileOutputStream(file.getPath()), StandardCharsets.UTF_8);
      out.write(quote.getQuote());
      out.close();
    } catch (IOException e) {
      LOG.log(Level.SEVERE, "Could not write to file {0}", e.getMessage());
    }
  }
